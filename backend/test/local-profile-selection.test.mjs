...
import { localSelectionSettings, selectLocalProfile, setLocalReasoning, setLocalHostAddress, normalizeLocalHostAddress, controlLocalModel } from '../src/local-profile-selection.mjs';
...
test('model control resolves selected host and reasoning without modifying assignments or sessions',async()=>{
  const queries=[],calls=[];
  const args={runtime:{draining:false},characterID:'test',action:'stop',
    pool:{query:async(sql,values)=>{queries.push({sql,values});return {rows:[{definition,address:'192.168.1.15',reasoning:'off'}]};}},
    localProviders:{controlModel:async(d,action)=>{calls.push({d,action});return {state:'stopped',loaded:false};}}};
  assert.equal((await controlLocalModel(args)).state,'stopped');
  assert.equal(calls[0].d.host.address,'192.168.1.15');assert.equal(calls[0].d.profile.reasoning,'off');
  assert.equal(calls[0].action,'stop');assert.equal(queries.length,1);
  assert.match(queries[0].sql,/^SELECT/);assert.deepEqual(queries[0].values,['test']);
  await assert.rejects(controlLocalModel({...args,action:'delete'}),/Unsupported/);
  await assert.rejects(controlLocalModel({...args,pool:{query:async()=>({rows:[]})}}),/먼저 선택/);
...
test('local selection pins measured model and restores exact previous cloud settings/config',()=>{
  ...
  assert.equal(restored.autoCompactPercent,original.autoCompactPercent??90);
...
test('isolated PostgreSQL: select and restore end sessions without deleting history', {skip:process.env.OFFICESTRA_LOCAL_DB_TEST!=='1'}, async()=>{
  ...
  try {
    ...
    await client.query('ALTER TABLE characters ADD COLUMN auto_compact_percent smallint NOT NULL DEFAULT 30');
    ...
    assert.equal(row.auto_compact_percent,100);
    ...
    assert.equal(row.auto_compact_percent,30);
