...
- 따라 읽기 자식과 합성 작업자는 앱 번들과 별개인 `~/.officestra/voice`를 작업 폴더로 사용한다. 앱을 재빌드해 기존 번들 폴더가 교체돼도 모델 파일을 찾는 경로가 바뀌지 않는다.
- 토크나이저 적재나 예열이 실패하면 버튼을 켜진 상태로 유지하지 않고 실패로 표시한다. 모델 적재 경고와 실패 원인은 `follow.log`에서 확인할 수 있다.

## 소리가 나지 않을 때

버튼이 켜져 있는데도 합성 기록이 없거나 `Tokenizer not loaded`가 반복되면 `follow.log`를 확인한다. 앱을 재빌드하기 전에 시작한 백엔드는 삭제된 임시 번들 폴더를 작업 폴더로 붙들 수 있다. 이 경우 앱 화면의 **백엔드 재시작** 버튼으로 새 번들의 백엔드를 시작한 뒤 음성 지원을 다시 켠다. 음성 버튼만 껐다 켜서는 오래된 백엔드 작업 폴더가 바뀌지 않는다.
